...
Чтобы использовать proxy для загрузки плагинов указать в переменной PROXY_OPTS файла docker-compose.yaml адрес прокси и раскомментировать.<br>После того, как загрузите плагины, то закомментировать обратно.
