...
> Плагины с IP адресов РФ могут не загружаться, в таком случае можно использовать ваш рабочий proxy (если он у вас есть).<br>
Чтобы использовать proxy для загрузки плагинов, указать в переменной PROXY_OPTS файла docker-compose.yaml адрес и порт и раскомментировать.<br>После того, как загрузите плагины, то закомментировать обратно.
